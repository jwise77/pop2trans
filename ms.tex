...
We use the adaptive mesh refinement (AMR) code \enzo~\citep{refs} that
employs our implementation of adaptive ray tracing for radiation
transport \citep{refs}.  To resolve minihalos with at least 100 dark
matter (DM) particles and follow the formation of the first generation
of dwarf galaxies, we use a simulation box of 1 Mpc that has a
resolution of $256^3$.  This gives us a DM mass resolution of 1840
\Ms.  We refine the grid on baryon overdensities of $3 \times
...
cells.  The latter criterion avoids any artifical fragmentation during
gaseous collapses \citep{Truelove97}.  We initialize the simulation
with \textsl{grafic} \citep{Bertschinger01} at $z = ???$ and use the
cosmological parameters from the 7-year WMAP data \citep{ref}:
